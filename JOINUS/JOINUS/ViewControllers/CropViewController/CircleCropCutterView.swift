...
class CircleCropCutterView: UIView {
  ...
  override func draw(_ rect: CGRect) {
    ...
    let circle = UIBezierPath(ovalIn: CGRect(x: rect.size.width/2 - length/2, y: rect.size.height/2 - length/2,
    ...
    //This is the same rect as the UIScrollView size 240 * 240, remains centered
    let square = UIBezierPath(rect: CGRect(x: rect.size.width/2 - length/2, y: rect.size.height/2 - length/2,
                                           width: length, height: length))
